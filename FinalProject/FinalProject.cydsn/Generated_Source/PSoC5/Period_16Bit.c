...
/*******************************************************************************
* Function Name: Period_16Bit_Start
********************************************************************************
*
* Summary:
...
void Period_16Bit_Start(void) 
{
    ...
    Period_16Bit_CLKEN |= Period_16Bit_CLKEN_MASK;
	Period_16Bit_CLKSTBY |= Period_16Bit_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Period_16Bit_Stop
********************************************************************************
*
* Summary:
...
void Period_16Bit_Stop(void) 
{
    ...
    Period_16Bit_CLKEN &= (uint8)(~Period_16Bit_CLKEN_MASK);
	Period_16Bit_CLKSTBY &= (uint8)(~Period_16Bit_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Period_16Bit_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Period_16Bit_CLKEN & Period_16Bit_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Period_16Bit_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Period_16Bit_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Period_16Bit__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Period_16Bit_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Period_16Bit_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Period_16Bit_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Period_16Bit_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Period_16Bit_GetSourceRegister();
    uint16 oldDivider = Period_16Bit_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Period_16Bit_CLKEN & Period_16Bit_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Period_16Bit_DIV_PTR, clkDivider);
                Period_16Bit_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Period_16Bit_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Period_16Bit_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Period_16Bit_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Period_16Bit_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Period_16Bit_DIV_PTR);
}


/*******************************************************************************
* Function Name: Period_16Bit_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Period_16Bit_MOD_SRC |= modeBitMask & (uint8)Period_16Bit_MODE_MASK;
}


/*******************************************************************************
* Function Name: Period_16Bit_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Period_16Bit_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Period_16Bit_MODE_MASK));
}


/*******************************************************************************
* Function Name: Period_16Bit_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Period_16Bit_MOD_SRC & (uint8)(Period_16Bit_MODE_MASK);
}


/*******************************************************************************
* Function Name: Period_16Bit_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Period_16Bit_GetDividerRegister();
    uint8 oldSrc = Period_16Bit_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Period_16Bit_MOD_SRC |= CYCLK_SSS;
        Period_16Bit_MOD_SRC =
            (Period_16Bit_MOD_SRC & (uint8)(~Period_16Bit_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Period_16Bit_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Period_16Bit_MOD_SRC & Period_16Bit_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Period_16Bit_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Period_16Bit_PHASE = clkPhase & Period_16Bit_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Period_16Bit_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Period_16Bit_PHASE & Period_16Bit_PHASE_MASK;
}
