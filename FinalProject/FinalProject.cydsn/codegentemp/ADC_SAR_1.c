...
***************************************/
#if(CY_PSOC5A)
    static uint8 ADC_SAR_1_resolution;
#endif /* End CY_PSOC5A */
...
void ADC_SAR_1_Stop(void)
{
    ...
    ADC_SAR_1_SAR_CSR0_REG &= (uint8)~ADC_SAR_1_SAR_SOF_START_CONV;
        ...
        ADC_SAR_1_SAR_CSR6_REG &= (uint8)~ADC_SAR_1_INT_BYPASS_EXT_VREF;
        /* Disable reference buffer and reduce the reference power to the minimum */
        ADC_SAR_1_SAR_CSR3_REG &= (uint8)~ADC_SAR_1_SAR_EN_BUF_VREF_EN; /*VCM buffer is absent in PSOC5A*/
...
int8 ADC_SAR_1_GetResult8( void )
{
        ...
        return( (int8)((int16)res - ADC_SAR_1_shift) );
    #else
...
int16 ADC_SAR_1_CountsTo_mVolts(int16 adcCounts)
{
    ...
    mVolts = (int16)(( (int32)adcCounts * 1000 ) / ADC_SAR_1_countsPerVolt) ;
...
float32 ADC_SAR_1_CountsTo_Volts(int16 adcCounts)
{
    ...
    volts = (float32)adcCounts / (float32)ADC_SAR_1_countsPerVolt;
